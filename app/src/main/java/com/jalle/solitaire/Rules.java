...
class NormalSolitaire extends Rules {
  ...
  public void Resize(int width, int height) {
    int rem = width - App.CARD_WIDTH*7;
    int maxHeight = height - (20 + App.CARD_HEIGH);
    rem /= 8;
    for (int i = 0; i < 7; i++) {
      mCardAnchor[i+6].SetPosition(rem + i * (rem+App.CARD_WIDTH), 20 + App.CARD_HEIGH);
      mCardAnchor[i+6].SetMaxHeight(maxHeight);
    }

    for (int i = 3; i >= 0; i--) {
      mCardAnchor[i+2].SetPosition(rem + (6-i) * (rem + App.CARD_WIDTH), 10);
    }

    for (int i = 0; i < 2; i++) {
      mCardAnchor[i].SetPosition(rem + i * (rem + App.CARD_WIDTH), 10);
    }
...
class Spider extends Rules {
  ...
  public void Resize(int width, int height) {
    int rem = (width - (App.CARD_WIDTH * 10)) / 10;
    for (int i = 0; i < 10; i++) {
      mCardAnchor[i].SetPosition(rem/2 + i * (rem + App.CARD_WIDTH), 10);
...
class Freecell extends Rules {
  ...
  public void Resize(int width, int height) {
    int rem = (width - (App.CARD_WIDTH * 8)) / 8;
    for (int i = 0; i < 8; i++) {
      mCardAnchor[i].SetPosition(rem/2 + i * (rem + App.CARD_WIDTH), 10);
      mCardAnchor[i+8].SetPosition(rem/2 + i * (rem + App.CARD_WIDTH), 30 + App.CARD_HEIGH);
      mCardAnchor[i+8].SetMaxHeight(height - 30 - App.CARD_HEIGH);
    }
...
class FortyThieves extends Rules {
  ...
  public void Resize(int width, int height) {
    int rem = (width - (App.CARD_WIDTH * 10)) / 10;
    for (int i = 0; i < 10; i++) {
      mCardAnchor[i].SetMaxHeight(height - 30 - App.CARD_HEIGH);
      mCardAnchor[i].SetPosition(rem/2 + i * (rem + App.CARD_WIDTH), 30 + App.CARD_HEIGH);
      
      mCardAnchor[i+10].SetPosition(rem/2 + i * (rem + App.CARD_WIDTH), 10);
    }
